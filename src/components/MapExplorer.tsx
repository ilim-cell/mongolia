    ...
    <section className="section-padding bg-secondary/30">
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          <div className="order-2 lg:order-1 relative aspect-[4/5] md:aspect-[16/10] lg:aspect-[4/5] rounded-3xl overflow-hidden high-contrast-shadow">
            <Image
          ...
          <div className="order-1 lg:order-2 space-y-12">
            <div className="space-y-4">
              ...
              <p className="text-lg text-muted-foreground font-light leading-relaxed">{uiText["ui.mapExplorer.description"]}</p>
            </div>

            <div className="space-y-6 lg:space-y-10">
              {landmarks.map((landmark, idx) => {
                return (
                  <div key={idx} className="group p-6 lg:p-8 bg-white border border-border rounded-2xl hover:border-foreground transition-all duration-300">
                    <div className="flex items-center gap-4 mb-4">
                      ...
                      <h3 className="text-lg lg:text-xl font-bold">{landmark.title}</h3>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {landmark.badges.map((b, bIdx) => (
                        <Badge key={bIdx} variant="outline" className="text-[9px] font-bold uppercase tracking-widest px-3 py-1 border-border bg-white text-muted-foreground rounded-full">
                           {b}
                        </Badge>
                      ))}
                    </div>
