...
export function MapExplorer() {
  ...
  return (
    <section className="section-padding bg-slate-50 overflow-hidden w-full">
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-24">
          <div className="order-2 lg:order-1 relative aspect-[4/5] overflow-hidden bg-slate-200 border border-black/5">
            <Image
              src={Images.extra.map}
              alt="Central Map Region"
              fill
              unoptimized
              className="object-cover grayscale"
              data-ai-hint="mongolia topographic map satellite"
            />
            <div className="absolute top-8 left-8">
              <div className="glass-dark text-white px-6 py-2 text-[10px] font-bold uppercase tracking-[0.4em]">
                Regional Node Archive
              </div>
            </div>
          </div>
          ...
          <div className="order-1 lg:order-2 space-y-12">
            <div className="space-y-6">
              ...
              <p className="text-lg md:text-xl text-black/50 font-light max-w-md leading-relaxed">{uiText["ui.mapExplorer.description"]}</p>
            </div>
              ...
              {landmarks.map((landmark, idx) => (
                <div key={idx} className="p-10 bg-white border border-black/10 hover:border-black transition-all">
                  <div className="flex items-center gap-6 mb-6">
                    ...
                    <h3 className="text-2xl font-black uppercase tracking-tight">{landmark.title}</h3>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {landmark.badges.map((b, bIdx) => (
                      <Badge key={bIdx} variant="outline" className="text-[9px] font-bold uppercase tracking-[0.2em] px-4 py-1.5 border-black/10 rounded-none bg-slate-50">
                         {b}
                      </Badge>
                    ))}
                  </div>
